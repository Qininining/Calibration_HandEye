...
dx_fixed = 800;     % 固定主点偏移x (px)
dy_fixed = 600;     % 固定主点偏移y (px)

% ================== 初始猜测值 ==================
% T 矩阵参数的初始猜测 [Rz, Rx, Ry, tx, ty, tz]
initial_T_params = [-4.1, 0.0, 0.0, 7.99e6, -1.25e7, 2.57e6];
% M 矩阵参数的初始猜测 [sx, sy]
initial_M_params = [1000/820, 1000/820]; % 仅包含 sx, sy 的初始猜测

% 组合所有优化参数的初始猜测
initial_guess = [initial_T_params, initial_M_params]; % 总共 8 个参数
...
fprintf('  M: sx=%.4f, sy=%.4f (nm/px)\n', ...
        initial_guess(7), initial_guess(8));
fprintf('  M (固定): dx=%.1f, dy=%.1f (px)\n', dx_fixed, dy_fixed);


%% 构建目标函数，求解最优参数 (优化 T 和 M 的部分参数: Rz,Rx,Ry,tx,ty,tz,sx,sy)
% 准备齐次像素坐标 (4xn)
...
% 定义残差函数 (用于 lsqnonlin)
% params = [Rz, Rx, Ry, tx, ty, tz, sx, sy] - 8 parameters
% 该函数计算 T*M*points_pix_homo 与 points_world_real 之间的差异
% 注意：Zf, dx_fixed, dy_fixed 作为固定参数传入
residual_func = @(params) calculate_residuals_TM_fixed_dxy(params, points_pix_homo, points_world_real, Zf, dx_fixed, dy_fixed);
...
fprintf('开始使用 lsqnonlin 进行优化 (优化 T 和 M 的 sx, sy)...\n');
% 求解优化问题
...
% M 的参数 (固定的部分)
dx_optimized = dx_fixed;
dy_optimized = dy_fixed;
...
fprintf('sx = %.8f (nm/px)\n', sx_optimized);
fprintf('sy = %.8f (nm/px)\n', sy_optimized);
fprintf('dx = %.6f (px) (fixed)\n', dx_optimized);
fprintf('dy = %.6f (px) (fixed)\n', dy_optimized);
...
%% ================== 辅助函数 ==================
% 新的残差计算函数 (优化 T 和 M 的 sx, sy, 固定 dx, dy)
function residuals = calculate_residuals_TM_fixed_dxy(params, points_pix_homo, points_world_real, Zf, dx_fixed, dy_fixed)
    % params: [Rz, Rx, Ry, tx, ty, tz, sx, sy] - 8 parameters
    % T 参数
    Rz = params(1); Rx = params(2); Ry = params(3);
    tx = params(4); ty = params(5); tz = params(6);
    % M 参数 (优化的部分)
    sx = params(7); sy = params(8);
    % M 参数 (固定的部分)
    dx = dx_fixed;
    dy = dy_fixed;
    ...
    % 构建当前参数下的 M 矩阵 (使用优化的 sx, sy 和固定的 dx, dy, Zf)
